...
% Preallocate
bestSeed_grid   = zeros(nvox,1);    % best-fitting dictionary index from grid search
bestSeed_perm   = zeros(nvox,1);    % best-fitting dictionary index from permutation approach
...
for itcv = 1:(kfold + 1)
    ...
    [~, bestCol]    = max(cXtrFtr, [], 2);          % best-fitting column index
    bestSeed_grid   = bestCol;                      % store results (overwrites each pass, as an example)
    ...
    bestSeed_perm = locRanks; % store results
end
